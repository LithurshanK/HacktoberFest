...
int truckTour(vector<vector<int>> petrolpumps) {
    ...
    int petrolIn = 0; // petrol remains in tank after filling in a station and going to the next one.
    long unsigned int count = 1;
    
    while(count <= petrolpumps.size()){
        //if i is out of index but the circle not finished
        if (i==petrolpumps.size()){
            i=0;
        }
    
        petrolIn+=(petrolpumps[i][0]-petrolpumps[i][1]);
        count++;
        i++;
        
        // resetting starting point and petrolIn if the circle can't be finished with previous start.
        if (petrolIn<0){
            count = 1;
            start ++;
            i = start;
